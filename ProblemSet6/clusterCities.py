...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        single_link_dist = 100000000
        for p1 in self.points:
            for p2 in other.points:
                dist = p1.distance(p2)
                if dist < single_link_dist:
                    single_link_dist = dist
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        joint_points = c1.points + c2.points
        merged_cluster = Cluster(joint_points, c1.pointType)
        self.add(merged_cluster)
    ...
    def findClosest(self, linkage):
        ...
        most_sim = None
        for index, cluster1 in enumerate(self.getClusters()[:-1]):
            for cluster2 in self.getClusters()[index+1:]:
                distance = linkage(cluster1, cluster2)
                if distance < min_dist:
                    min_dist = distance
                    most_sim = (cluster1, cluster2)
        return most_sim
    ...
    def mergeOne(self, linkage):
        ...
        to_merge = self.findClosest(linkage)
        self.mergeClusters(to_merge[0], to_merge[1])
        return to_merge
